  ...
  <div class="basic">
    <div class="content">
      ...
      <div class="form">
        ...
        <div class="actions">
          <button class="btn primary" :disabled="!canSave" @click="saveSlot">{{ form.id ? 'Update' : 'Create' }} Slot</button>
      ...
      <div class="generator">
        ...
        <p class="gen-desc">Create a baseline set of shifts for all teams, based on our standard festival schedule. You can edit or disable any slot after generation.</p>
        <div class="grid gen">
          <label>Year</label>
        ...
        <div class="actions">
          <button class="btn primary" @click="generateAll" :disabled="generating">{{ generating ? 'Generating…' : 'Generate All Slots' }}</button>
        </div>
        <small class="hint">Rules: Setup 8h (10–18) Aug 24–Sep 4; Front Gate 2h (10–24) Sep 4–6; Food 4h (11–15, 17–21) Sep 4–6; Stage 4h blocks 08–24 and 00–04 (next day) Sep 4–6; Cleanup 8h (10–18) Sep 7.</small>
      </div>

      <div class="slots">
        <h3>Slots</h3>
        <div class="slot-row" v-for="s in filteredSlots" :key="s.id">
          <div class="meta">
          ...
          <div class="controls">
            ...
            <button class="btn" @click="toggleActive(s)">{{ s.active !== false ? 'Disable' : 'Enable' }}</button>
            <button class="btn danger" @click="deleteSlot(s)">Delete</button>
          </div>
        </div>
        <div v-if="!filteredSlots.length" class="empty">No slots found.</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    canSave() {
      return !!(this.form.team && this.form.date && this.form.start && this.form.end && this.form.capacity > 0)
    },
    filteredSlots() {
      const list = this.slots.slice().sort((a,b) => (a.date + a.start).localeCompare(b.date + b.start))
      if (this.teamFilter === 'all') return list
      return list.filter(s => s.team === this.teamFilter)
    }
  },
  created() {
    this.reload()
  ...
  methods: {
    ...
    editSlot(s) {
      this.form = { id: s.id, team: s.team, date: s.date, start: s.start, end: s.end, capacity: s.capacity || 1, notes: s.notes || '' }
    },
    async saveSlot() {
      const payload = {
        team: this.form.team,
      ...
      if (this.form.id) {
        ...
        await addDoc(collection(this.db, 'volunteer_slots_2026'), { ...payload, created_at: serverTimestamp(), claimed: [] })
      }
      this.resetForm()
    },
    async toggleActive(s) {
      await updateDoc(doc(this.db, 'volunteer_slots_2026', s.id), { active: !(s.active !== false), updated_at: serverTimestamp() })
    },
    async deleteSlot(s) {
    ...
    toDateStr(year, month, day) { return `${year}-${this.pad(month)}-${this.pad(day)}` },
    dateRange(start, end) {
      const out = []
    ...
    async generateAll() {
      ...
      try {
        ...
        const add = async (slot) => addDoc(collection(this.db, 'volunteer_slots_2026'), { ...slot, active: true, created_at: serverTimestamp(), updated_at: serverTimestamp(), claimed: [] })
        ...
        for (const date of setupDates) {
          await add({ team: 'setupcrew', date, start: '10:00', end: '18:00', capacity: this.gen.capacity.setup, notes: 'Setup (8h)' })
        }
        ...
        for (const date of fgDates) {
          ...
          while (t !== '00:00') {
            ...
            await add({ team: 'frontgate', date, start: t, end: end === '00:00' ? '24:00' : end, capacity: this.gen.capacity.frontgate, notes: 'Front Gate (2h)' })
            t = end
            if (t === '00:00') break
            if (t === '24:00') { t = '00:00' }
          }
        ...
        for (const date of foodDates) {
          await add({ team: 'foodteam', date, start: '11:00', end: '15:00', capacity: this.gen.capacity.food, notes: 'Food Team (Lunch)' })
          await add({ team: 'foodteam', date, start: '17:00', end: '21:00', capacity: this.gen.capacity.food, notes: 'Food Team (Supper)' })
        }
    ...
    for (const date of stageDates) {
      ...
      await add({ team: 'stagecrew', date, start: t, end, capacity: this.gen.capacity.stage, notes: 'Stage Crew (4h)' })
            t = end
            if (t === '24:00') { t = '00:00' }
          }
      ...
      await add({ team: 'cleanupcrew', date: cleanupDate, start: '10:00', end: '18:00', capacity: this.gen.capacity.cleanup, notes: 'Cleanup (8h)' })
          // 00–04 next-day block
           const [Y,M,D] = date.split('-').map(Number)
           const nextDate = this.dateRange(new Date(Y, M-1, D+1), new Date(Y, M-1, D+1))[0]
       await add({ team: 'stagecrew', date: nextDate, start: '00:00', end: '04:00', capacity: this.gen.capacity.stage, notes: 'Stage Crew (overnight 4h)' })
        }
...
.highlight { text-align: center; color: var(--reunion-frog-green, #4caf50); margin: 0 0 0.75rem; }
.toolbar { display: flex; gap: 0.5rem; align-items: center; margin: 0.5rem 0 1rem; }
.form { border: 1px solid #444; border-radius: 10px; padding: 1rem; margin-bottom: 1rem; }
.form .grid { display: grid; grid-template-columns: 140px 1fr; gap: 0.5rem 1rem; align-items: center; }
.form input, .form select { padding: 0.5rem; border-radius: 6px; border: 1px solid #444; background: #1e1e1e; color: #fff; }
.actions { margin-top: 0.5rem; display: flex; gap: 0.5rem; }
.generator { border: 1px solid #444; border-radius: 10px; padding: 1rem; margin-bottom: 1rem; background: #1a1a1a; }
.gen-title { margin: 0 0 0.25rem; }
.gen-desc { margin: 0 0 0.75rem; color: #ccc; }
.gen { grid-template-columns: 180px 1fr !important; }
.hint { display: block; margin-top: 0.5rem; opacity: 0.8; }
.slots { border: 1px solid #444; border-radius: 10px; padding: 1rem; }
.slot-row { display: grid; grid-template-columns: 2fr 1fr auto; gap: 0.75rem; align-items: center; padding: 0.75rem; border-bottom: 1px solid #333; }
.slot-row:last-child { border-bottom: none; }
.controls button { margin-left: 0.25rem; }

/* Buttons */
.btn { padding: 0.5rem 0.75rem; border-radius: 10px; border: 1px solid #666; background: #2a2a2a; color: #fff; cursor: pointer; transition: all 0.2s ease; }
.btn:hover { filter: brightness(1.1); }
.btn:disabled { opacity: 0.6; cursor: not-allowed; }
.btn.primary { border-color: var(--reunion-frog-green, #4caf50); background: rgba(76,175,80,0.12); }
.btn.outline { background: transparent; border-color: #888; }
.btn.danger { background: #8b0000; border-color: #c00; }
</style>
